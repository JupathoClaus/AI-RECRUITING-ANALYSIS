...
describe('FilesService', () => {
  ...
  beforeEach(async () => {
    ...
    storage = {
      ...
      put: jest.fn().mockResolvedValue({ storageKey: 'company-1/uuid-abc.pdf', checksumSha256: 'abc123', sizeBytes: 100 }),
      get: jest.fn().mockResolvedValue({ stream: {} as any, mimeType: 'application/pdf', sizeBytes: 100 }),
  ...
  describe('uploadResume', () => {
    it('should reject empty buffer', async () => {
      await expect(
        service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, Buffer.alloc(0), 'resume.pdf', 'application/pdf'),
    ...
    it('should reject oversized file', async () => {
      ...
      await expect(
        service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, large, 'resume.pdf', 'application/pdf'),
      ).rejects.toThrow(BadRequestException);
    });

    it('should reject missing extension', async () => {
      await expect(
        service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, pdfBuffer, 'resume', 'application/pdf'),
      ).rejects.toThrow(BadRequestException);
    });

    it('should reject blocked extension', async () => {
      await expect(
        service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, Buffer.from('something'), 'resume.exe', 'application/x-msdownload'),
      ).rejects.toThrow(BadRequestException);
    });

    it('should reject spoofed MIME type', async () => {
      await expect(
        service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, pdfBuffer, 'resume.pdf', 'text/html'),
      ).rejects.toThrow(BadRequestException);
    });

    it('should reject PDF signature mismatch', async () => {
      await expect(
        service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, textBuffer, 'resume.pdf', 'application/pdf'),
      ).rejects.toThrow(BadRequestException);
    });

    it('should reject DOCX signature mismatch', async () => {
      await expect(
        service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, Buffer.from('not a zip'), 'resume.docx', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should reject non-existent application', async () => {
      ...
      await expect(
        service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, pdfBuffer, 'resume.pdf', 'application/pdf'),
      ).rejects.toThrow(NotFoundException);
    });

    it('should accept valid PDF', async () => {
      prisma.application.findFirst.mockResolvedValue({ id: mockApplicationId, companyId: mockCompanyId });
      ...
      const result = await service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, pdfBuffer, 'resume.pdf', 'application/pdf');
    ...
    it('should accept valid DOCX', async () => {
      ...
      prisma.storedFile.findFirst.mockResolvedValue(null);
      prisma.storedFile.updateMany.mockResolvedValue({ count: 0 });
      prisma.storedFile.create.mockResolvedValue({ id: 'file-2', storageKey: 'key', originalName: 'res.docx' });
      prisma.storedFile.findUnique.mockResolvedValue({ id: 'file-2' });

      const result = await service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, docxBuffer, 'res.docx', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
      expect(result).toBeDefined();
    });

    it('should handle replacement by superseding previous', async () => {
      prisma.application.findFirst.mockResolvedValue({ id: mockApplicationId, companyId: mockCompanyId });
      prisma.storedFile.findFirst.mockResolvedValueOnce({ id: 'old-file' }).mockResolvedValueOnce(null);
      prisma.storedFile.updateMany.mockResolvedValue({ count: 1 });
      prisma.storedFile.create.mockResolvedValue({ id: 'new-file', storageKey: 'company-1/new.pdf' });
      prisma.storedFile.findUnique.mockResolvedValue({ id: 'new-file' });

      const result = await service.uploadResume(mockApplicationId, mockCompanyId, mockUserId, pdfBuffer, 'new.pdf', 'application/pdf');
  ...
  describe('downloadFile', () => {
    it('should reject non-existent file', async () => {
      ...
      await expect(
        service.downloadFile('nonexistent', mockCompanyId, mockUserId),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should reject soft-deleted file', async () => {
      ...
      await expect(
        service.downloadFile('file-1', mockCompanyId, mockUserId),
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe('archiveFile', () => {
    it('should reject non-existent file', async () => {
      prisma.storedFile.findUnique.mockResolvedValue(null);
      await expect(
        service.archiveFile('nonexistent', mockCompanyId, mockUserId),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should reject cross-tenant archive', async () => {
      ...
      await expect(
        service.archiveFile('file-1', mockCompanyId, mockUserId),
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe('getApplicationResume', () => {
    it('should reject non-existent application', async () => {
      prisma.application.findFirst.mockResolvedValue(null);
      await expect(
        service.getApplicationResume('bad-app', mockCompanyId),
      ).rejects.toThrow(NotFoundException);
    });
