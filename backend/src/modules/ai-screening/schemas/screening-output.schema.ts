...
const ALLOWED_RECOMMENDATIONS: ReadonlySet<string> = new Set(Object.values(ScreeningRecommendation));
const ALLOWED_CONFIDENCE: ReadonlySet<string> = new Set(Object.values(ScreeningConfidence));
const ALLOWED_SOURCE_CATEGORIES: ReadonlySet<string> = new Set(Object.values(ScreeningSourceCategory));
...
function checkNoUnknownKeys(obj: Record<string, unknown>, allowed: Set<string>, context: string): void {
...
export function validateScreeningOutput(value: unknown): ProviderScreeningResult {
  ...
  if (typeof recommendation !== 'string' || !ALLOWED_RECOMMENDATIONS.has(recommendation)) {
    throw new AiScreeningMalformedResponseError(`recommendation must be one of: ${Array.from(ALLOWED_RECOMMENDATIONS).join(', ')}`);
  }

  const confidence = obj.confidence;
  if (typeof confidence !== 'string' || !ALLOWED_CONFIDENCE.has(confidence)) {
    throw new AiScreeningMalformedResponseError(`confidence must be one of: ${Array.from(ALLOWED_CONFIDENCE).join(', ')}`);
  }
  ...
  for (const item of evidence) {
    ...
    if (typeof ev.sourceCategory !== 'string' || !ALLOWED_SOURCE_CATEGORIES.has(ev.sourceCategory)) {
      throw new AiScreeningMalformedResponseError(`Each evidence item must have a valid sourceCategory, got '${ev.sourceCategory}'`);
    }
  ...
  for (const cs of criteriaScores) {
    ...
    if (typeof cso.criterion !== 'string' || cso.criterion.trim().length === 0) {
      throw new AiScreeningMalformedResponseError('Each criteriaScore must have a non-empty criterion');
    }
