...
const SCENARIO_MARKER_PREFIX = '__MOCK_SCENARIO:';
...
export class MockScreeningProvider implements AiScreeningProvider {
  ...
  private detectScenario(input: ScreeningInput): MockScenario | null {
    if (!input.jobDescription.startsWith(SCENARIO_MARKER_PREFIX)) return null;
    const marker = input.jobDescription.slice(SCENARIO_MARKER_PREFIX.length).trim() as MockScenario;
    if ((['STRONG_MATCH', 'WEAK_MATCH', 'INSUFFICIENT_EVIDENCE', 'PROHIBITED_REASONING', 'MALFORMED_RESULT', 'PROVIDER_FAILURE'] as MockScenario[]).includes(marker)) {
      return marker;
    }
    return null;
  }
  ...
  private defaultResult(input: ScreeningInput): ProviderScreeningResult {
    ...
    const matchRatio = input.requiredSkills.length > 0 ? matched.length / input.requiredSkills.length : 0;
    ...
    if (matchRatio >= 0.6) {
      return {
        ...
        missingQualifications: input.requiredSkills.filter((s) => !resumeText.includes(s.toLowerCase())),
        ...
        criteriaScores: [
          { criterion: 'skills_match', score: Math.round(matchRatio * 100), maximumScore: 100, weight: 0.5, explanation: 'Based on required skill match ratio' },
          { criterion: 'experience', score: 50, maximumScore: 100, weight: 0.3, explanation: 'Experience match from resume' },
          { criterion: 'education', score: 50, maximumScore: 100, weight: 0.2, explanation: 'Education match from resume' },
        ],
    ...
    if (matchRatio >= 0.3) {
      return {
        ...
        uncertainties: [`Candidate matches only ${matched.length}/${input.requiredSkills.length} required skills`],
        riskFlags: [],
        explanation: `Candidate matches ${matched.length}/${input.requiredSkills.length} required skills. Human review recommended.`,
        criteriaScores: [
          { criterion: 'skills_match', score: Math.round(matchRatio * 100), maximumScore: 100, weight: 0.5, explanation: 'Partial skills match' },
        ],
        prohibitedReasoningDetected: false,
        providerMetadata: { provider: 'mock', promptVersion: 'v1' },
      };
    ...
    return {
      ...
      criteriaScores: [
        { criterion: 'skills_match', score: Math.round(matchRatio * 100), maximumScore: 100, weight: 0.5, explanation: 'Low skills match' },
      ],
      prohibitedReasoningDetected: false,
      providerMetadata: { provider: 'mock', promptVersion: 'v1' },
    };
  ...
  private strongMatch(): ProviderScreeningResult {
    return {
      ...
      recommendation: ScreeningRecommendation.SHORTLIST,
      confidence: ScreeningConfidence.HIGH,
      matchedQualifications: ['Strong technical background', 'Relevant industry experience', 'Required skills verified'],
      missingQualifications: [],
      evidence: [
        { criterion: 'technical_skills', sourceCategory: ScreeningSourceCategory.RESUME, sourceText: '10+ years experience in relevant technologies', assessment: 'strong_match', score: 95, weight: 0.5, isRequired: true },
        { criterion: 'domain_experience', sourceCategory: ScreeningSourceCategory.RESUME, sourceText: 'Previous role in similar industry', assessment: 'strong_match', score: 90, weight: 0.3, isRequired: true },
      ],
      uncertainties: [],
      riskFlags: [],
      explanation: 'Candidate strongly matches all required qualifications with verified experience.',
      criteriaScores: [
        { criterion: 'technical_skills', score: 95, maximumScore: 100, weight: 0.5, explanation: 'Excellent technical alignment' },
        { criterion: 'experience', score: 90, maximumScore: 100, weight: 0.3, explanation: 'Strong experience match' },
        { criterion: 'education', score: 85, maximumScore: 100, weight: 0.2, explanation: 'Education meets requirements' },
      ],
      prohibitedReasoningDetected: false,
      providerMetadata: { provider: 'mock', promptVersion: 'v1' },
    };
  ...
  private weakMatch(): ProviderScreeningResult {
    return {
      ...
      missingQualifications: ['Required technical skills not demonstrated', 'Insufficient relevant experience', 'Missing required education'],
      evidence: [
        { criterion: 'technical_skills', sourceCategory: ScreeningSourceCategory.JOB_REQUIREMENT, sourceText: 'Required: TypeScript, React, Node.js', assessment: 'missing', isRequired: true },
        { criterion: 'experience', sourceCategory: ScreeningSourceCategory.JOB_REQUIREMENT, sourceText: 'Required: 5+ years experience', assessment: 'missing', isRequired: true },
      ],
      uncertainties: [],
      ...
      criteriaScores: [
        { criterion: 'technical_skills', score: 10, maximumScore: 100, weight: 0.5, explanation: 'Required skills not found in resume' },
        { criterion: 'experience', score: 15, maximumScore: 100, weight: 0.3, explanation: 'Experience below minimum requirements' },
        { criterion: 'education', score: 50, maximumScore: 100, weight: 0.2, explanation: 'Partial education match' },
      ],
      prohibitedReasoningDetected: false,
      providerMetadata: { provider: 'mock', promptVersion: 'v1' },
    };
  ...
  private insufficientEvidence(): ProviderScreeningResult {
    return {
      ...
      missingQualifications: [],
      evidence: [
        { criterion: 'evidence_availability', sourceCategory: ScreeningSourceCategory.UNKNOWN, sourceText: 'Insufficient evidence to evaluate', assessment: 'insufficient', isRequired: true },
      ],
      ...
      criteriaScores: [
        { criterion: 'overall', score: 0, maximumScore: 100, weight: 1, explanation: 'Cannot evaluate due to insufficient data' },
      ],
      prohibitedReasoningDetected: false,
      providerMetadata: { provider: 'mock', promptVersion: 'v1' },
    };
  ...
  private prohibitedReasoning(): ProviderScreeningResult {
    return {
      ...
      missingQualifications: [],
      evidence: [
        { criterion: 'compliance', sourceCategory: ScreeningSourceCategory.UNKNOWN, sourceText: 'Prohibited reasoning detected in provider output', assessment: 'flag', isRequired: true },
      ],
