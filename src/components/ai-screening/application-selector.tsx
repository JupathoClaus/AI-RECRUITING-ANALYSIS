...
interface Props {
  onSelect: (applicationId: string, hasResume: boolean) => void
...
export function ApplicationSelector({ onSelect, selectedId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const items: ApplicationItem[] = (result.data ?? []).map((app: any) => ({
        id: app.id,
        ...
        hasResume: app.resumeFiles ? app.resumeFiles.length > 0 : false,
    ...
    <div className="space-y-3">
      ...
      <div className="max-h-96 overflow-y-auto space-y-1">
        {filtered.map((app) => (
          <button
            key={app.id}
            onClick={() => onSelect(app.id, app.hasResume)}
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge variant={app.hasResume ? 'success' : 'secondary'}>{app.hasResume ? 'Resume OK' : 'No Resume'}</Badge>
