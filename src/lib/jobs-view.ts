...
export const JOB_ACTIVE_STATUSES = ["DRAFT", "PENDING_APPROVAL", "APPROVED", "SCHEDULED", "PUBLISHED", "PAUSED"]

export const JOB_HISTORY_STATUSES = ["CLOSED", "FILLED", "CANCELLED", "ARCHIVED"]

export type JobScope = "active" | "history"

export function resolveJobStatusFilter(scope: JobScope, explicitStatus: string | null): string[] {
  const allowed = scope === "history" ? JOB_HISTORY_STATUSES : JOB_ACTIVE_STATUSES
  if (explicitStatus && explicitStatus !== "all" && allowed.includes(explicitStatus)) {
